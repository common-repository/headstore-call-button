...
$title_campaign = __('If you have multiple "PAYCALL Buttons" the Campaign ID lets you identify which one is in use. For example if you use the same button on several sites, you can determine which site the call came from.', $this->plugin_name);

$title_widget_title = __('This title will appear above your "PAYCALL Button".', $this->plugin_name);

$title_username = __('If you have multiple PAYCALL accounts, you can chose one here.', $this->plugin_name);
...
<?php 
	...
	{ 
		?>
		No PAYCALL buttons found for this Username.<br />
		Go to <a href="https://<?php echo $this->headstoreBackendSubdomain;?>.headstore.com/#signup">headstore.com</a> to create your first PAYCALL button.<br /><br /><br />
    ...
    if(!$hasButtons)
		{ 
			...
			?>
		No PAYCALL buttons found for this Account.<br />
		Go to <a target="_blank" href="<?php echo hs_backend::domain;?>/">headstore.com</a> to create your first PAYCALL button.<br /><br /><br />
	...
	<p>
		...
		<?php echo ($hasGroups?(($expertOrGroup['dataFrom']=="INDIVIDUAL_EXPERT"?"Profile":"Group").": "):"").$expertOrGroup['shortDescription']; ?></option>
                <?php } ?>
                </select>  
...
<p>
	<span title='<?php echo $title_campaign;?>'><label for="<?php echo $this->get_field_id(widget_call_me_fields::campaign); ?>"><?php _e('Give your "PAYCALL Button" an identifier (optional):', $this->plugin_name); ?></label></span>
