...
<div class="hs-meta-boxes-account">
	...
	<div class="hs-fields-boxes">
		<div class="wrap">
		   ...
		   <?php if($message_create) { ?>
				<h4><?php _e( 'Login to an existing PAYCALL account:', $this->plugin_name); ?></h4>
				<?php _e( 'If you already have another PAYCALL account, please enter the email address registered to the account.', $this->plugin_name); ?> 
		   <?php } else { ?>
		...
		<div class="wrap">
		 ...
		 <?php if($message_create) { ?>
		 <div class="wrap">
		    <h4><?php _e('Create a new PAYCALL account:',$this->plugin_name); ?></h4>
    ...
    <?php if($token) { ?>
		<div class="hs-expert-list">
			 <h3><?php _e( "Profiles and group", $this->plugin_name); ?></h3>
			<?php if($experts_list) { ?>
				<select multiple disabled>
				  <?php echo $options;?>
				</select>
		     <?php } else {?>
				 <?php _e( "The account doesn't have profiles and groups", $this->plugin_name); ?>
			 <?php } ?>
	   </div>
    <?php } ?>
	 -->
