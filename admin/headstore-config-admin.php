...
class Headstore_Config_Admin {
	...
	public function add_post_type_account() {
		...
		$accounts = __( 'Accounts', $this->plugin_name );
		  ...
		  register_post_type( account_meta_data::post_type,
		    array(
		      'labels' => array(
				'menu_name' =>__( 'PAYCALL Button', $this->plugin_name ),
		        'name' => $accounts,
				'all_items' => $accounts,
	...
	public function add_meta_box_account(){
		...
		add_meta_box( account_meta_data::post_type.'_meta', __( 'PAYCALL Account Info', $this->plugin_name), array( $this, 'display_meta_form_account' ), account_meta_data::post_type, 'advanced', 'high' );
	...
	public function my_admin_notices_save_post() {
		...
		switch ($hs_message) {
		    case 0:
		        $message = __("Congratulations! You successfully connected your PAYCALL account with this Wordpress plugin. Now you can place 'PAYCALL Buttons' as",$this->plugin_name);
	...
	public function hs_mce_plugin( $plugins ) {
		...
		wp_localize_script(
			...
			array(
				...
				'label_expert' => __( 'Profile and group :', $this->plugin_name ),
				'label_campaign' => __( 'Give your "PAYCALL Button" an identifier (optional):', $this->plugin_name ),
			     'label_campaign_input_title' => __("If you have multiple 'PAYCALL Buttons' the Campaign ID lets you identify which one is in use. For example if you use the same button on several sites, you can determine which site the call came from.", $this->plugin_name),
				 'label_username_input_title' => __('If you have multiple PAYCALL accounts, you can chose one here.', $this->plugin_name),
				...
				'id_ajaxurl' => site_url('/').'wp-admin/admin-ajax.php',
				'label_expert_input_title' => __( 'Chose one of your groups. You can create new ones logging in with your account on https://paycall.headstore.com', $this->plugin_name ),
				'label_design_input_title' =>  __( 'Chose one design', $this->plugin_name ),
				'label_expert_subinfo' =>  __('It can take a few seconds to load your profile after the account has been chosen.', $this->plugin_name),
			)
		);
	...
	public function headstore_posts_columns($columns) {
		...
		$columns['status']= __( 'Connected with PAYCALL', $this->plugin_name);
		return $columns;
	...
	public function hs_check_user_ajax() {
		...
		if($count > 0) {
	 	    ...
	 	    print_r(json_encode(array("response" => 'error',
	 	    	                      "message" => __("This PAYCALL account is already connected to the WordPress plugin. All the profiles and their 'PAYCALL Buttons' can already be accessed though the already connected account.", $this->plugin_name))));
			exit;
